...
<div class="container">
    <a href="CreateNewCustomer" class="create-btn">+ Create New Customer</a>
    <input type="text" id="searchInput" class="search-bar" placeholder="Search by ID, Name, Email, or Phone..." onkeyup="filterTable()">
    ...
    <table id="customersTable">
        ...
        <%
        List<Customers> c = (List<Customers>) request.getAttribute("customer");
        if(c != null && !c.isEmpty()){
            for(Customers i : c){
        %>
        <tr>
            ...
            <td><a class="action-btn edit-btn" href="EditCustomerServlet?customer_id=<%=i.getCust_id()%>">Edit</a></td>
            <td><a href="#" class="action-btn delete-btn" onclick="confirmDelete(<%=i.getCust_id()%>)">Delete</a></td>
        </tr>
        <%  }
        } %>
    </table>
...
function confirmDelete(customerId) {
    Swal.fire({
        ...
        if(result.isConfirmed) {
            window.location.href = 'DeleteCustomerServlet?customer_id=' + customerId + '&deleted=true';
        }
    });
}
...
function filterTable() {
    ...
    for (let i = 1; i < tr.length; i++) { // skip header row
        ...
        let found = false;
        for (let j = 0; j < 4; j++) { // check first 4 columns: ID, Name, Email, Phone
